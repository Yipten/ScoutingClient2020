...
namespace ScoutingClient2020.ViewModels {
	class TeamStatsViewModel {
		...
		public TeamStatsViewModel() {
			...
			AvgAllTotal = new Stat("SELECT AVG(AutoBottom + AutoOuter + AutoInner + TeleBottom + TeleOuter + TeleInner) FROM RawData WHERE TeamNumber = {0};", "Total");
			...
			AvgAutoTotal = new Stat("SELECT AVG(AutoBottom + AutoOuter + AutoInner) FROM RawData WHERE TeamNumber = {0};", "Total");
			...
			AvgTeleTotal = new Stat("SELECT AVG(TeleBottom + TeleOuter + TeleInner) FROM RawData WHERE TeamNumber = {0};", "Total");
